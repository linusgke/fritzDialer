...
public class Action {
    ...
    private final TR064Connection connection;
    private final ServiceDesc serviceXML;

    public Action(ActionType action, List<StateVariableType> stateVariableList, TR064Connection connection, ServiceDesc serviceXML) {
        ...
        for (StateVariableType s : stateVariableList) {
            Class<?> type = null;
            if ("string".equals(s.getDataType())) type = String.class;
            else if (s.getDataType().startsWith("ui") || s.getDataType().startsWith("i")) type = Integer.class;
            else if ("boolean".equals(s.getDataType())) type = Boolean.class;
            else if ("dateTime".equals(s.getDataType())) type = Date.class;
            else if ("uuid".equals(s.getDataType())) type = UUID.class;
            else {
                log.error("UNKNOWN TYPE: {} {} {}", s.getDataType(), s.getName(), action.getName());
            }
            stateToType.put(s.getName(), type);
        }
        if (action.getArgumentList() != null) for (ArgumentType a : action.getArgumentList()) {
    ...
    public boolean isOut(String argument) {
        if (!argumentOut.containsKey(argument)) throw new UnsupportedOperationException(argument);
    ...
    public Response execute(Map<String, Object> arguments) throws IOException, UnauthorizedException {
        ...
        if (arguments != null) {
            for (String a : arguments.keySet()) {
                ...
                if (argumentOut.get(a)) throw new UnsupportedOperationException(a + " is not In");
                if (parameter.getClass() != this.getTypeOfArgument(a)) {
                    try {
                        parameter = Class.forName(this.getTypeOfArgument(a).getName()).cast(parameter);
                    } catch (Exception e) {
                        throw new UnsupportedOperationException(a + " has to be " + this.getTypeOfArgument(a) + ". Cast not possible: " + e.getMessage(), e);
                    }
        ...
        if (!inArguments.isEmpty()) throw new UnsupportedOperationException("Missing In-Arguments: " + inArguments);
        ...
        try {
            ...
            SOAPBodyElement element = body.addBodyElement(envelope.createName(this.name, "u", serviceXML.getServiceType()));
            if (arguments != null) {
        ...
        return getMessage(connection.getSOAPXMLIS(serviceXML.getControlURL(), serviceXML.getServiceType() + "#" + this.getName(), httpEntity));
    }
