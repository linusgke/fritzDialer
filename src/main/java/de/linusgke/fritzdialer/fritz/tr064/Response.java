...
public class Response {
    ...
    public Response(SOAPMessage response, Map<String, Class<?>> stateToType, Map<String, String> argumentState) throws SOAPException {
        ...
        for (int i = 1; i < tmp.getLength() && nodes == null; i++) {
            if ("#text".equals(tmp.item(i).getNodeName())) continue;
            nodes = tmp.item(i).getChildNodes();
        }

        if (nodes != null) {
            for (int i = 1; i < nodes.getLength(); i++) {
                if ("#text".equals(nodes.item(i).getNodeName())) continue;
    ...
    public int getValueAsInteger(String argument) throws ClassCastException, NoSuchFieldException {
        if (!argumentState.containsKey(argument) || !data.containsKey(argument))
            throw new NoSuchFieldException(argument);
        if (stateToType.get(argumentState.get(argument)) != Integer.class) throw new ClassCastException(argument);
    ...
    public boolean getValueAsBoolean(String argument) throws ClassCastException, NoSuchFieldException {
        if (!argumentState.containsKey(argument) || !data.containsKey(argument))
            throw new NoSuchFieldException(argument);
        if (stateToType.get(argumentState.get(argument)) != Boolean.class) throw new ClassCastException(argument);

        boolean ret;

        if ("1".equals(data.get(argument)) || "true".equalsIgnoreCase(data.get(argument))) ret = true;
        else if ("0".equals(data.get(argument)) || "false".equalsIgnoreCase(data.get(argument))) ret = false;
        else throw new ClassCastException(argument);
    ...
    public Date getValueAsDate(String argument) throws ClassCastException, NoSuchFieldException {
        if (!argumentState.containsKey(argument) || !data.containsKey(argument))
            throw new NoSuchFieldException(argument);
        if (stateToType.get(argumentState.get(argument)) != Date.class) throw new ClassCastException(argument);
    ...
    public UUID getValueAsUUID(String argument) throws NoSuchFieldException {
        if (!argumentState.containsKey(argument) || !data.containsKey(argument))
            throw new NoSuchFieldException(argument);
        if (stateToType.get(argumentState.get(argument)) != UUID.class) throw new ClassCastException(argument);
